...
void computeHog(unsigned char image_buffer[32][64], ap_int<15> descriptor[72 * 4], int sum_of_block[8]){
...
#pragma HLS ARRAY_PARTITION variable=tmp_binsum complete dim=1
	...
	for(int x = 0; x < 64; x++){
		unsigned char pix = image_buffer[0][x];
		linebuf.insert_bottom_row(pix, x);
	}

	linebuf.shift_pixels_up(0);
	linebuf.insert_bottom_row(image_buffer[1][0], 0);
	for(int x = 1; x < 64; x++){
	...
	#pragma HLS PIPELINE II=9
					...
					int y = cy * 8 + yy;
					if(y < 31){
						unsigned char pix = image_buffer[(x == 63 ? y + 2 : y + 1)][x == 63 ? 0 : x + 1];
...
void normalizeBlock(int sum_of_block[8], ap_int<15> descriptor[HISTOGRAMSIZE], unsigned short normalized[HISTOGRAMSIZE]){
	//int  average;
...
#pragma HLS ARRAY_PARTITION variable=image_buffer complete dim=1
	read_input:for(int i = 0; i < HEIGHT; i++){
		int offset = i * 64;
		memcpy(image_buffer[i], imageptr + offset, (WIDTH) * sizeof(unsigned char));
	}
	ap_int<15>  descriptor[HISTOGRAMSIZE]={0};
	unsigned short	  normalized[HISTOGRAMSIZE]={0};
#pragma HLS RESOURCE variable=normalized core=RAM_2P_BRAM
//#pragma HLS ARRAY_PARTITION variable=descriptor0 complete dim=1
...
#pragma HLS PIPELINE II=1
	...
	computeHog(image_buffer, descriptor, sum_of_block);
